...
xkb_write_keymap_for_names_cb(FILE *out, void *userdata)
{
    ...
    if (xkbDebugFlags) {
        ...
        XkbWriteXKBKeymapForNames(stderr, global_ctx->names, global_ctx->xkb, global_ctx->want, global_ctx->need);
    }
#endif
...
//    log("XkbDDXNamesFromRules %s", rules_name);
    if (!rules_name)
        return FALSE;
...
//        log("XKB: Rules name is too long\n");
        return FALSE;
    }
...
//        log("XKB: Couldn't open rules file %s\n", buf);
        return FALSE;
    }
...
//        log("XKB: Couldn't create rules struct\n");
        fclose(file);
        return FALSE;
    }
...
//        log("XKB: Couldn't parse rules file %s\n", rules_name);
...
//        log("XKB: Rules returned no components\n");
    }
...
XkbCompileKeymapForDevice(DeviceIntPtr dev, XkbRMLVOSet * rmlvo, int need)
{
    ...
    char name[PATH_MAX];
//    log("XkbCompileKeymapForDevice0 %p", xkb);

    if (XkbRMLVOtoKcCGST(dev, rmlvo, &kccgst)) {
        provided =
            XkbDDXLoadKeymapByNames(dev, &kccgst, XkmAllIndicesMask, need, &xkb,
                                    name, PATH_MAX);
//        log("XkbCompileKeymapForDevice %p", xkb);
