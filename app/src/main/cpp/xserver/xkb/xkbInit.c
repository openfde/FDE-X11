...
#define log(...) if(PRINT_LOG){\
                __android_log_print(ANDROID_LOG_DEBUG, "native_xkb", __VA_ARGS__);\
                }              \

#define loge(...) if(PRINT_LOG){\
                __android_log_print(ANDROID_LOG_ERROR, "native_xkb", __VA_ARGS__);\
                }
                                 ...
                                 BellProcPtr bell_func, KbdCtrlProcPtr ctrl_func)
{
    ...
    log("InitKeyboardDeviceStructInternal %s", keymap);
