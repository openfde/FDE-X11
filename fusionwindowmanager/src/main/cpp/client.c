...
clientConfigureWindows (Client *c, XWindowChanges * wc, unsigned long mask, unsigned short flags)
{
    ...
    {
        change_values.x = c->x;//frameX (c);
        change_values.y = c->y;//frameY (c);
    ...
    {
        change_values.x = 0;//frameLeft (c);
        change_values.y = 0;//frameTop (c);
        change_values.width = c->width;
        change_values.height = c->height;
...
clientFrame (DisplayInfo *display_info, Window w, gboolean recapture)
{
    ...
    c->hint_class.res_name = NULL;
    c->hint_class.res_class = NULL;
    XGetClassHint (display_info->dpy, w, &c->hint_class);
    ...
    //TODO FDE
    XReparentWindow (display_info->dpy, c->window, c->frame, 0, 0);
...
out:
    ...
    // XCompositeNameWindowPixmap(display_info->dpy, c->frame);
...
clientFrameAll (ScreenInfo *screen_info)
{
    ...
    logd ("screen_info->xroot %lx", screen_info->xroot);
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if ((attr.map_state == IsViewable) && (attr.root == screen_info->xroot) && wins[i] != screen_info->xfwm4_win)
        {
            clientFrame (display_info, wins[i], TRUE);
        }
    }
    if (wins)
    {
...
clientAddToList (Client * c)
{
    ...
    screen_info = c->screen_info;
    display_info = screen_info->display_info;
    display_info->clients = g_slist_append (display_info->clients, c);
