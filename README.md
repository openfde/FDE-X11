...
The build runs on both a linux and a windows host.
...
* Android SDK with NDK 25.1.8937393 and CMake 3.22.1 (see `local.properties`),
* a JDK that can read the class files of the flavor's
  `app/libs/<flavor>/framework.jar`, which is used as the javac boot class path:
  `versionc` is built by a JDK 21 toolchain (class file 65.0), `versionu` needs
  Java 17 and `versionr` still Java 7/8, so use a JDK 21 for all of them. Set it
  as `JAVA_HOME` or as `org.gradle.java.home` in the user level
  `%GRADLE_USER_HOME%\gradle.properties`; in Android Studio use
  *Settings > Build, Execution, Deployment > Build Tools > Gradle > Gradle JDK*
  and pick a JDK 21 (the IDE overrides `gradle.properties`). Building with an
  older JDK fails with a message naming the jar and the required version.
* bison, e.g. `choco install winflexbison` (`win_bison.exe` has to be on the
  `PATH`), and Python 3 for the generated keysym tables.

Then build from `cmd`/PowerShell, e.g.
``
gradlew.bat assembleVersioncDebug
``

Notes on the windows build:

* `app/src/main/cpp` no longer needs a POSIX shell: the sources in `patches/`
  are applied with `git apply` (falling back to GNU `patch`), `ks_tables.h` is
  generated by `makekeys.py` instead of a host `gcc`, and the `X11/Xtrans`
  headers are copied instead of symlinked (symlinks need extra privileges).
* `libx11/include/X11/Xlocale.h` is called `XlibLocale.h` because a
  case-insensitive filesystem made bionic's `<xlocale.h>` include it by mistake.
* pixman's ARM NEON assembly needs the GNU cross assembler, which only exists on
  a linux host, so the windows arm64 build uses pixman's portable C fast paths.
...
`versionu` and `versionr` also need an updated `app/libs/<flavor>/framework.jar`
(one that carries `com.android.internal.policy.ITaskCaptionOperationService`)
and a `FrameworkFactory` matching the flavor's `FrameworkImpl` constructor; in
this revision only `versionc` is self-consistent.
